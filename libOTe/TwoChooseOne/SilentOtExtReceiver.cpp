...
void SilentOtExtReceiver::genBaseOts(PRNG& prng, Channel& chl)
{
...
void SilentOtExtReceiver::genSilentBaseOts(PRNG& prng, Channel& chl)
{
    if (isConfigured() == false)
        throw std::runtime_error("configure must be called first");
    ...
    for (u64 i = 0; i < mS.size(); ++i)
    {
        if (mS[i] >= mN2)
        {
            ...
            {
                std::cout << Color::Red << "bad " << j << " " << mS[j] << " / " << mN2 << std::endl
                          << Color::Default;
                std::terminate();
            }
        }
...
void SilentOtExtReceiver::genBase(
    u64 n, Channel& chl, PRNG& prng, u64 scaler, u64 secParam, SilentBaseType basetype, u64 threads)
{
    switch (basetype)
    {
...
void SilentOtExtReceiver::configure(u64 n, u64 scaler, u64 secParam, u64 numThreads)
{
    mP = nextPrime(n);
    mN = roundUpTo(mP, 128);
    mScaler = scaler;
...
void SilentOtExtReceiver::checkRT(span<Channel> chls, Matrix<block>& rT1)
{
    ...
    for (u64 i = 0; i < R.rows(); ++i)
    {
        if (neq(R(i), exp(i)))
        {
            std::cout << i << " / " << R.rows() << " R= " << R(i) << " exp= " << exp(i)
                      << std::endl;
            throw RTE_LOC;
        }
    }
...
void SilentOtExtReceiver::silentReceive(
    BitVector& choices, span<block> messages, PRNG& prng, Channel& chl)
{
    silentReceive(choices, messages, prng, {&chl, 1});
}

void SilentOtExtReceiver::silentReceive(
    BitVector& choices, span<block> messages, PRNG& prng, span<Channel> chls)
{
    if (isConfigured() == false)
    {
...
void SilentOtExtReceiver::randMulQuasiCyclic(
    Matrix<block>& rT, span<block>& messages, BitVector& choices, u64 threads)
{
    ...
    auto routine = [&](u64 index) {
        ...
        auto multAddReduce = [this, nBlocks, n64, &a, &bPoly, &cPoly, &temp128, &cache](
                                 span<block> b128, span<block> dest) {
        ...
        // for (u64 i = index; i < numBlocks; i += threads)
        {
            ...
            for (u64 k = 0; k < 128; k += 8)
            {
                mAesFixedKey.ecbEncBlocks(
                    tpBuffer.data() + k, hashBuffer.size(), hashBuffer.data());
    ...
    //			lout << "count[" << i << "] " << counts[i][0] << " " << counts[i][1] << " " <<
    //counts[i][2] << " " << counts[i][3] << std::endl;

    //		lout << "total " << totals[0] << " " << totals[1] << " " << totals[2] << " " << totals[3]
    //<< std::endl;
}
