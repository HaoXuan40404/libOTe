...
void SilentOtExtSender::setSlientBaseOts(span<std::array<block, 2>> sendBaseOts)
{
...
void SilentOtExtSender::configure(u64 n, u64 scaler, u64 secParam, u64 numThreads)
{
    mP = nextPrime(n);
    mN = roundUpTo(mP, 128);
    mScaler = scaler;
...
void SilentOtExtSender::send(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl)
{
...
void SilentOtExtSender::silentSend(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl)
{
    silentSend(messages, prng, {&chl, 1});
}

void SilentOtExtSender::silentSend(
    span<std::array<block, 2>> messages, PRNG& prng, span<Channel> chls)
{
    if (isConfigured() == false)
    {
...
void bitShiftXor(span<block> dest, span<block> in, u8 bitShift)
{
    ...
    if (bitShift >= 64)
    {
        ...
        {
            block b0 = toBlock(inPtr), b1 = toBlock(inPtr + sizeof(u64));
...
void SilentOtExtSender::randMulQuasiCyclic(
    Matrix<block>& rT, span<std::array<block, 2>>& messages, u64 threads)
{
    auto nBlocks = mN / 128;
    ...
    auto routine = [&](u64 index) {
        ...
        // for (u64 i = index; i < numBlocks; i += threads)
        {
            ...
            for (; k < min; ++k)
            {
                ...
                messages[j + k][1] =
                    mAesFixedKey.ecbEncBlock(tpBuffer[k] ^ mGen.mValue) ^ tpBuffer[k] ^ mGen.mValue;
            }
