...
void KosOtExtSender::send(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl)
{
    if (hasBaseOts() == false)
        genBaseOts(prng, chl);
    ...
    for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
    {
        ...
        while (mIter != mEnd)
        {
            while (mIter != mEnd && tIter < tEnd)
            {
                ...
                // std::cout << "midx " << (mIter - messages.data()) << "   tIdx " << tIdx <<
                // std::endl;
        ...
        while (xIter != xEnd)
        {
            while (xIter != xEnd && tIter < tEnd)
            {
                *xIter = *tIter;
                ...
                // u64 tV = tIter - (block*)t.data();
                // u64 tIdx = tV / 8 + (tV % 8) * 128;
                // std::cout << "xidx " << (xIter - extraBlocks.data()) << "   tIdx " << tIdx <<
                // std::endl;
        ...
        for (u64 rowIdx = doneIdx, j = 0; j < xx; ++rowIdx, ++j)
        {
...
#error "OTE_KOS_HASH" must be defined
    ...
    for (u64 blockIdx = 0; blockIdx < bb; ++blockIdx)
    {
        ...
        for (u64 i = 0, dd = doneIdx; dd < stop; ++dd, ++i)
        {
            ...
            // std::cout << "sendIdx' " << dd << "   " << messages[dd][0] << "   " << chii <<
            // std::endl;
