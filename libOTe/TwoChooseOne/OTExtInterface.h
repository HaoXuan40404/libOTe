...
public:
    ...
    virtual void receive(
        const BitVector& choices, span<block> messages, PRNG& prng, Channel& chl) = 0;
...
public:
    ...
    // messages.
    virtual void send(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl) = 0;

    // send chosen strings. Thosen strings are read from messages.
    void sendChosen(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl);

    template <typename CorrelationFunc>
    void sendCorrelated(
        span<block> messages, const CorrelationFunc& corFunc, PRNG& prng, Channel& chl)
    {
