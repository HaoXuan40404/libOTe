...
void KosDotExtSender::send(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl)
{
    if (hasBaseOts() == false)
        genBaseOts(prng, chl);
    ...
    for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
    {
        ...
        if (mIter >= mIterPartial)
        {
            ...
            auto xCount = std::min<u64>(
                128 * superBlkSize - mCount, extraBlocks.data() + extraBlocks.size() - xIter);
        ...
        {
            MatrixView<u8> tOut((u8*)&*mIter, 128 * superBlkSize, sizeof(block) * 2);
    ...
    block qi1, qi2, qi3, qi4, q1(ZeroBlock), q2(ZeroBlock), q3(ZeroBlock), q4(ZeroBlock);
    ...
    for (u64 blockIdx = 0; blockIdx < bb; ++blockIdx)
    {
        ...
        for (; dd < stop0; ++dd, ++i)
        {
            mul256(messages[dd][0], messages[dd][1], challenges[i], challenges2[i], qi1, qi2, qi3,
                qi4);
        ...
        {
            mul256(extraBlocks[xx][0], extraBlocks[xx][1], challenges[i], challenges2[i], qi1, qi2,
                qi3, qi4);
            q1 = q1 ^ qi1;
            q2 = q2 ^ qi2;
            q3 = q3 ^ qi3;
