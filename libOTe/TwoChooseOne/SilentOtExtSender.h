...
public:
    ...
    std::unique_ptr<OtExtSender> split() override { throw std::runtime_error("not impl"); }

    // use the default base OT class to generate the
    // IKNP base OTs that are required.
    void genBaseOts(PRNG& prng, Channel& chl) override { mIknpSender.genBaseOts(prng, chl); }
    ...
    // OTs in various ways.
    void genBase(u64 n, Channel& chl, PRNG& prng, u64 scaler = 2, u64 secParam = 128,
    ...
    void silentSend(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl);

    // A parallel exection version of the other
    // silentSend(...) function.
    void silentSend(span<std::array<block, 2>> messages, PRNG& prng, span<Channel> chls);
