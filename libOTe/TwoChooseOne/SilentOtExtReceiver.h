...
public:
    ...
    u64 baseOtCount() const override { return mIknpRecver.baseOtCount(); }

    // returns true if the IKNP base OTs are currently set.
    bool hasBaseOts() const override { return mIknpRecver.hasBaseOts(); };

    // Returns an indpendent copy of this extender.
    virtual std::unique_ptr<OtExtReceiver> split() { throw std::runtime_error("not implemented"); };
    ...
    void configure(u64 n, u64 scaler = 2, u64 secParam = 128, u64 numThreads = 1);
    ...
    void genBase(u64 n, Channel& chl, PRNG& prng, u64 scaler = 2, u64 secParam = 80,
        SilentBaseType base = SilentBaseType::BaseExtend, u64 threads = 1);
    ...
    void silentReceive(BitVector& choices, span<block> messages, PRNG& prng, Channel& chl);

    // A parallel version of the other silentReceive(...)
    // function.
    void silentReceive(BitVector& choices, span<block> messages, PRNG& prng, span<Channel> chls);
