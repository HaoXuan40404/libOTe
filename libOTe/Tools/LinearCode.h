...
public:
    ...
    inline u64 codewordBitSize() const { return mCodewordBitSize; }

    inline u64 codewordBlkSize() const { return (codewordBitSize() + 127) / 128; }
    inline u64 plaintextBitSize() const { return mG.size() / codewordBlkSize(); }
    inline u64 plaintextBlkSize() const { return (plaintextBitSize() + 127) / 128; }

    inline u64 plaintextU8Size() const { return mPlaintextU8Size; }

    inline u64 codewordU8Size() const { return (codewordBitSize() + 7) / 8; }
