...
void LinearCode::writeBinCppFile(const std::string& fileName, const std::string& name)
{
    ...
    int c = 0;
    out.read((char*)&c, 1);
    fOut << "static unsigned char " << name << "[] = { 0x" << std::hex << std::setw(2)
         << std::setfill('0') << c;
...
void LinearCode::generateMod8Table()
{
    ...
    if (plaintextU8Size() > sLinearCodePlainTextMaxSize)
    {
        std::cout << "The encode function assumes that the plaintext word"
                     " size is less than "
                  << sLinearCodePlainTextMaxSize
                  << ". If"
                     " this is not the case, raise the limit"
                  << std::endl;
        throw std::runtime_error(LOCATION);
    }
...
void LinearCode::encode(const span<block>& plaintxt, const span<block>& codeword)
{
#ifndef NDEBUG
    ...
    //
    //        std::array<block, 8>
    //            c{ ZeroBlock ,ZeroBlock ,ZeroBlock ,ZeroBlock,ZeroBlock ,ZeroBlock ,ZeroBlock
    //            ,ZeroBlock };
    //
    //
...
void LinearCode::encode(const span<u8>& plaintxt, const span<u8>& codeword)
{
#ifndef NDEBUG
