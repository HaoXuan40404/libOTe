...
namespace bpm
{
inline void __xor_down_256(__m256i* poly, u64 dest_idx, u64 src_idx, u64 len)
{
    for (u64 i = len; i > 0;)
    {
        i--;
...
// USED;
inline void __xor_up_256(__m256i* poly, u64 dest_idx, u64 src_idx, u64 len)
{
    for (u64 i = 0; i < len; i++)
    {
        poly[dest_idx + i] = xor256(poly[dest_idx + i], poly[src_idx + i]);
...
inline void __xor_down_256_2(__m256i* poly, u64 len, u64 l_st)
{
    __xor_down_256(poly, l_st, len, len);
}

inline void xor_up_256(__m256i* poly, u64 st, u64 len, u64 diff)
{
    __xor_up_256(poly, st, diff + st, len);
}

inline void __xor_up_256_2(__m256i* poly, u64 len, u64 l_st)
{
...
__m256i (*_sh_op[8])(__m256i h, __m256i l) = {_mm256_alignr_255bit, _mm256_alignr_254bit,
    _mm256_alignr_252bit, _mm256_alignr_31byte, _mm256_alignr_30byte, _mm256_alignr_28byte,
    _mm256_alignr_24byte, _mm256_alignr_16byte};

// USED;

__m256i (*_sh_op_zerohigh[8])(__m256i h, __m256i l) = {_mm256_alignr_255bit_zerohigh,
    _mm256_alignr_254bit_zerohigh, _mm256_alignr_252bit_zerohigh, _mm256_alignr_31byte,
    _mm256_alignr_30byte, _mm256_alignr_28byte, _mm256_alignr_24byte, _mm256_alignr_16byte};


// USED;
inline void __sh_xor_down(__m256i* poly256, u64 unit, u64 _op, __m256i zero)
{
    u64 unit_2 = unit >> 1;
    poly256[unit_2] = xor256(poly256[unit_2], _sh_op_zerohigh[_op](zero, poly256[unit - 1]));
    for (u64 i = 0; i < unit_2 - 1; i++)
    {
        poly256[unit_2 - 1 - i] = xor256(
...
void varsub_x256(__m256i* poly256, u64 n_256)
{
    ...
    while (log_n > 8)
    {
        ...
        u64 unit_2 = unit >> 1;
        for (u64 j = 0; j < num; j++)
            __xor_down_256_2(poly256 + j * unit, unit_2, (1ull << (log_n - 9)));
        log_n--;
    }
    ...
    for (u64 i = log_n; i > 0; i--)
    {
        ...
        u64 num = n_256 / unit;
        for (u64 j = 0; j < num; j++)
            __sh_xor_down(poly256 + j * unit, unit, i - 1, zero);
    }
}


// USED;
inline void __sh_xor_up(__m256i* poly256, u64 unit, u64 _op, __m256i zero)
{
    u64 unit_2 = unit >> 1;
    poly256[0] = xor256(poly256[0], _sh_op[_op](poly256[unit_2], zero));
    for (u64 i = 0; i < unit_2 - 1; i++)
    {
        poly256[i + 1] =
            xor256(poly256[i + 1], _sh_op[_op](poly256[unit_2 + i + 1], poly256[unit_2 + i]));
    }
    poly256[unit_2] = xor256(poly256[unit_2], _sh_op_zerohigh[_op](zero, poly256[unit - 1]));
}
...
void i_varsub_x256(__m256i* poly256, u64 n_256)
{
    ...
    for (u64 i = 1; i <= _log_n; i++)
    {
        ...
        u64 num = n_256 / unit;
        for (u64 j = 0; j < num; j++)
            __sh_xor_up(poly256 + j * unit, unit, i - 1, zero);
    }
    ...
    for (u64 i = 9; i <= log_n; i++)
    {
        ...
        u64 unit_2 = unit >> 1;
        for (u64 j = 0; j < num; j++)
            __xor_up_256_2(poly256 + j * unit, unit_2, (1ull << (i - 9)));
    }
}
