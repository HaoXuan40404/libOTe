...
{
// USED;
inline void bit_bc_64x2_div(__m256i* x)
{
    for (u64 i = 0; i < 64; i++)
        x[64 + i] = _mm256_srli_si256(x[i], 8);
    ...
    for (u64 i = 0; i < 64; i++)
        x[i] = _mm256_unpacklo_epi64(x[i], x[64 + i]);
...
void encode_128_half_input_zero(uint64_t* rfx, const uint64_t* fx, u64 n_fx_128b)
{
    ...
    for (u64 i = 0; i < num; i++)
    {
        ...
        for (u64 j = 0; j < 8; j++)
        {
            bitmatrix_prod_64x128_4R_b32_opt_avx2((uint8_t*)(&t2_128[32 * j]),
                beta_mul_64_bm4r_ext_8, (const uint8_t*)(&temp64[32 * j]));
        }
        for (u64 k = 0; k < 8; k++)
            for (u64 j = 0; j < 8; j++)
                rfx_128[i * 256 + k * 8 + j] = t2_128[k * 32 + j * 2];
        for (u64 k = 0; k < 8; k++)
            for (u64 j = 0; j < 8; j++)
                rfx_128[i * 256 + 64 + k * 8 + j] = t2_128[k * 32 + 16 + j * 2];
        for (u64 k = 0; k < 8; k++)
            for (u64 j = 0; j < 8; j++)
                rfx_128[i * 256 + 128 + k * 8 + j] = t2_128[k * 32 + j * 2 + 1];
        for (u64 k = 0; k < 8; k++)
            for (u64 j = 0; j < 8; j++)
                rfx_128[i * 256 + 128 + 64 + k * 8 + j] = t2_128[k * 32 + 16 + j * 2 + 1];
    }
}

// USED;
inline void bit_bc_exp(__m256i* x)
{
...
void decode_128(uint64_t* rfx, const uint64_t* fx, u64 n_fx_128b)
{
    ...
    for (u64 i = 0; i < num; i++)
    {
        ...
        for (u64 j = 0; j < 8; j++)
        {
            bitmatrix_prod_128x128_4R_b32_opt_avx2((uint8_t*)(temp + 16 * j),
                i_beta_mul_64_bm4r_ext_8, (const uint8_t*)(temp + 16 * j));
        }
...
inline void bit_bc_64x2_exp(__m256i* x)
{
    for (u64 i = 0; i < 64; i += 4)
    {
        __m256i* pi = x + i;
...
void encode_64_half_input_zero(uint64_t* rfx, const uint64_t* fx, u64 n_fx)  /// XXX: further
                                                                             /// optimization.
{
    if (64 * 4 > n_fx)
    {
...
#else
        for (u64 j = 0; j < (64 * 4 / 64); j++)
            bitmatrix_prod_64x64_h32zero_4R_b64_avx2(
                (uint8_t*)(&temp64[64 * j]), beta_mul_32_bm4r, (uint8_t*)(&temp64[64 * j]));
#endif

        for (u64 j = 0; j < 64; j++)
            rfx[i * 256 + j] = temp64[j * 4];
        for (u64 j = 0; j < 64; j++)
            rfx[i * 256 + 64 + j] = temp64[j * 4 + 1];
        for (u64 j = 0; j < 64; j++)
            rfx[i * 256 + 128 + j] = temp64[j * 4 + 2];
        for (u64 j = 0; j < 64; j++)
            rfx[i * 256 + 192 + j] = temp64[j * 4 + 3];
    }
}
...
void decode_64(uint64_t* rfx, const uint64_t* fx, u64 n_fx)
{
    ...
    for (u64 i = 0; i < num; i++)
    {
        ...
        // truncated iFFT
        for (u64 j = 0; j < (64 * 4 / 64); j++)
            bitmatrix_prod_64x64_4R_b64_avx2(
                (uint8_t*)(&temp64[64 * j]), i_beta_mul_32_bm4r, (uint8_t*)(&temp64[64 * j]));
