...
std::vector<u64> ToU64Vector(const std::vector<block>& blockArray) {
    ...
    for(u64 i = 0; i < blockArray.size(); i++) {
        std::uint64_t first = blockArray[i].as<std::uint64_t>()[0];
        std::uint64_t second = blockArray[i].as<std::uint64_t>()[1];
...
std::vector<block> ToBlockVector(const std::vector<u64>& array) {
    ...
    if(array.size()%2 != 0) {
        std::cout << "ToBlock length check failed, array.size() = "<< array.size() << std::endl;
        return vec;
    }
    for(u64 i = 0; i < array.size(); i+=2) {
        u64 first = array[i];
        u64 second = array[i+1];
        block num = block(second, first);
        vec.push_back(num);
    }
...
std::vector<u64> MatrixToU64Vector(const Matrix<block>& matrix) {
    ...
    {
        for (u64 j = 0; j < matrix.cols(); ++j)
        {
            std::uint64_t first = matrix[i][j].as<std::uint64_t>()[0];
            std::uint64_t second = matrix[i][j].as<std::uint64_t>()[1];
...
Matrix<block> U64VectorToMatrix(const std::vector<u64>& array, u64 rows, u64 cols) {
    ...
    for (u64 i = 0; i < rows; ++i)
    {
        ...
        {
            u64 index = (i*cols+j)*2;
            std::uint64_t first = array[index];
            std::uint64_t second = array[index+1];
            block num = block(second, first);
            matrix[i][j] = num;
        }
    }
...
{
    const static u64 TRANSPOSE_MASKS128[7][2] = {{0x0000000000000000, 0xFFFFFFFFFFFFFFFF},
        {0x00000000FFFFFFFF, 0x00000000FFFFFFFF}, {0x0000FFFF0000FFFF, 0x0000FFFF0000FFFF},
        {0x00FF00FF00FF00FF, 0x00FF00FF00FF00FF}, {0x0F0F0F0F0F0F0F0F, 0x0F0F0F0F0F0F0F0F},
        {0x3333333333333333, 0x3333333333333333}, {0x5555555555555555, 0x5555555555555555}};
        ...
        if (width < 64)
        {
            for (u32 j = 0; j < nswaps; j++)
            {
                for (u32 k = 0; k < width; k++)
                {
                    ...
                    d2 = (t2 & mask2) ^ ((tt2 & mask2) << width) ^ ((tt1 & mask1) >> (64 - width));
...
void transpose(const MatrixView<u8>& in, const MatrixView<u8>& out)
{
    ...
    auto step = in.stride();
    auto step01 = step * 1, step02 = step * 2, step03 = step * 3, step04 = step * 4,
         step05 = step * 5, step06 = step * 6, step07 = step * 7, step08 = step * 8,
         step09 = step * 9, step10 = step * 10, step11 = step * 11, step12 = step * 12,
    ...
    for (int h = 0; h < subBlockHight; ++h)
    {
        ...
        for (int w = 0; w < subBlockWidth; ++w)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                // use the special movemask_epi8 to perform the final step of that bit-wise
                // tranpose. this instruction takes ever 8'th bit (start at idx 7) and moves them
                // into a single 16 bit output. Its like shaving off the top bit of each of the 16
                // bytes.
    ...
    if (leftOverWidth)
    {
        for (int h = 0; h < subBlockHight; ++h)
        {
            ...
            std::array<u8*, 16> src{start, start + step01, start + step02, start + step03,
                start + step04, start + step05, start + step06, start + step07, start + step08,
                start + step09, start + step10, start + step11, start + step12, start + step13,
                start + step14, start + step15};
...
inline void sse_loadSubSquarex(
    array<array<block, 8>, 128>& in, array<block, 2>& out, u64 x, u64 y, u64 i)
{
...
inline void sse_transposeSubSquarex(
    array<array<block, 8>, 128>& out, array<block, 2>& in, u64 x, u64 y, u64 i)
{
