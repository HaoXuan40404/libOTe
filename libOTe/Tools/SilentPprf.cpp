...
// @mal         - ...
block copyOut(span<std::array<block, 8>> lvl, MatrixView<block> output, u64 totalTrees, u64 tIdx,
    bool transpose, bool mal)
{
    if (transpose)
    {
...
block SilentMultiPprfSender::expand(
    Channel& chl, block value, PRNG& prng, MatrixView<block> output, bool transpose, bool mal)
{
    return expand({&chl, 1}, value, prng, output, transpose, mal);
}

block SilentMultiPprfSender::expand(
    span<Channel> chls, block value, PRNG& prng, MatrixView<block> output, bool transpose, bool mal)
{
    setValue(value);
        ...
        for (u64 g = threadIdx * 8; g < mPntCount; g += 8 * chls.size())
        {
            ...
            for (u64 d = 0; d < mDepth - 1; ++d)
            {
                for (u64 j = 0; j < min; ++j)
                {
                    ...
                    if (mPrint)
                    {
                        std::cout << "c[" << g + j << "][" << d << "][0] " << sums[0][d][j] << " "
                                  << mBaseOTs[g + j][d][0] << std::endl;
                        ;
                        std::cout << "c[" << g + j << "][" << d << "][1] " << sums[1][d][j] << " "
                                  << mBaseOTs[g + j][d][1] << std::endl;
                        ;
                    }
#endif
...
block SilentMultiPprfReceiver::expand(
    Channel& chl, PRNG& prng, MatrixView<block> output, bool transpose, bool mal)
{
    return expand({&chl, 1}, prng, output, transpose, mal);
}

block SilentMultiPprfReceiver::expand(
    span<Channel> chls, PRNG& prng, MatrixView<block> output, bool transpose, bool mal)
{
    ...
    auto routine = [&](u64 threadIdx) {
        ...
        auto printLevel = [&](u64 d) {
            ...
            for (i64 i = 0; i < level0.size(); ++i)
            {
                for (u64 j = 0; j < 8; ++j)
                {
                    ...
                    std::cout << "p[" << i << "][" << j << "] " << level0[i][j] << " "
                              << flevel0[i][j] << std::endl
                              << Color::Default;
        ...
        for (u64 g = threadIdx * 8; g < mPntCount; g += 8 * chls.size())
        {
            ...
            TODO(
                "Optimize this recv so that if we have fewer than 8 trees then less data is "
                "sent..");
            ...
            for (u64 i = 0; i < min; ++i)
            {
                ...
                if (neq(l1[notAi][i], l1f[notAi][i]))
                {
                    std::cout << "l1[" << notAi << "][" << i << "] " << l1[notAi][i] << " = "
                              << (mBaseOTs[i + g][0]) << " ^ " << theirSums[notAi][0][i] << " vs "
                              << l1f[notAi][i] << std::endl;
                }
#endif
            }
            ...
            for (u64 d = 1; d < mDepth; ++d)
            {
                ...
                if (d != mDepth - 1)
                {
                    for (u64 i = 0; i < min; ++i)
                    {
                        ...
                        auto correctSum = inactiveChild ^ theirSums[notAi][d][i];

                        inactiveChild = correctSum ^ mySums[notAi][i] ^ mBaseOTs[i + g][d];

#ifdef DEBUG_PRINT_PPRF
                        if (mPrint)
                            std::cout << "up[" << i << "] = level1[" << inactiveChildIdx << "]["
                                      << i << "] " << prev << " -> " << level1[inactiveChildIdx][i]
                                      << " " << activeChildIdx << " " << inactiveChildIdx << " ~~ "
                                      << mBaseOTs[i + g][d] << " " << theirSums[notAi][d][i]
                                      << " @ " << (i + g) << " " << d << std::endl;
            ...
            for (u64 j = 0; j < min; ++j)
            {
                ...
                if (mPrint)
                    std::cout << "up[" << d << "] = level1[" << (inactiveChildIdx / mPntCount)
                              << "][" << (inactiveChildIdx % mPntCount) << " " << prev << " -> "
                              << level[inactiveChildIdx][j] << " ~~ " << mBaseOTs[j + g][d] << " "
                              << ot0 << " @ " << (j + g) << " " << d << std::endl;
#endif
            }
