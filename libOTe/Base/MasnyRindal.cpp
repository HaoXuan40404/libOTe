...
void MasnyRindal::receive(const BitVector& choices, span<block> messages, PRNG& prng, Channel& chl)
{
    ...
    for (u64 i = 0; i < n;)
    {
        ...
        for (u64 k = 0; k < curStep; ++k, ++i)
        {
            ...
            r[0].toBytes(sendBuffIter);
            sendBuffIter += pointSize;
            r[1].toBytes(sendBuffIter);
            sendBuffIter += pointSize;
        }
...
void MasnyRindal::send(span<std::array<block, 2>> messages, PRNG& prng, Channel& chl)
{
    ...
    for (u64 i = 0; i < n;)
    {
        ...
        for (u64 k = 0; k < curStep; ++k, ++i)
        {
            std::array<u8*, 2> buffIters{buffIter, buffIter + pointSize};
