...
void NaorPinkas::receive(
    const BitVector& choices, span<block> messages, PRNG& prng, Channel& socket, u64 numThreads)
{
    ...
    for (u64 t = 0; t < numThreads; ++t)
    {
        ...
        thrds[t] = std::thread([t, numThreads, &messages, seed, &sendBuff, &choices, cRecvFuture,
                                   &cBuff, &remainingPK0s, &PK0Prom, nSndVals, &RFuture, &R]() {
...
void NaorPinkas::send(
    span<std::array<block, 2>> messages, PRNG& prng, Channel& socket, u64 numThreads)
{
    ...
    for (u64 t = 0; t < numThreads; ++t)
    {
        thrds[t] = std::thread([t, seed, fieldElementSize, &messages, recvFuture, numThreads, &buff,
                                   &alpha, nSndVals, &pC, &socket, &R]() {
            Curve curve;
