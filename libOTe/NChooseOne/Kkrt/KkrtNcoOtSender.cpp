...
void  KkrtNcoOtSender::genBaseOtsStep1(PRNG& prng, std::vector<std::array<block, 2>>& msgs) {
    auto count = getBaseOTCount();
    std::vector<std::array<block, 2>> msgs_tmp(count);
    DefaultBaseOT base;
    msgs = msgs_tmp;

}

void KkrtNcoOtSender::setBaseOts(gsl::span<block> baseRecvOts, const BitVector& choices)
{
        ...
        for (u64 i = 0; i < numCols / 128; ++i)
        {
            ...
            for (u64 tIdx = 0, colIdx = i * 128; tIdx < 128; ++tIdx, ++colIdx)
            {
                mGens[colIdx].ecbEncCounterMode(
                    mGensBlkIdx[colIdx], superBlkSize, ((block*)t.data() + superBlkSize * tIdx));
...
void KkrtNcoOtSender::recvCorrection(Channel& chl, u64 recvCount)
{
    ...
    chl.recv((u8*)&*dest, recvCount * sizeof(block) * mCorrectionVals.stride());
