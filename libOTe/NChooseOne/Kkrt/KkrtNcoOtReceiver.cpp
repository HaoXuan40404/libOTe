...
namespace osuCrypto
{
void KkrtNcoOtReceiver::setBaseOts(gsl::span<std::array<block, 2>> baseRecvOts)
{
...
void KkrtNcoOtReceiver::initStep1(u64 numOtExt, block seed, u8* theirComm, block theirSeed)
{
    ...
    for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
    {
        ...
        u64 stopIdx = doneIdx + std::min<u64>(u64(128) * superBlkSize, numOtExt - doneIdx);
        ...
        for (u64 i = 0; i < numCols / 128; ++i)
        {
            for (u64 tIdx = 0, colIdx = i * 128; tIdx < 128; ++tIdx, ++colIdx)
            {
                ...
                mGens[colIdx][0].ecbEncCounterMode(
                    mGensBlkIdx[colIdx], superBlkSize, ((block*)t0.data() + superBlkSize * tIdx));
                mGens[colIdx][1].ecbEncCounterMode(
                    mGensBlkIdx[colIdx], superBlkSize, ((block*)t1.data() + superBlkSize * tIdx));
...
void KkrtNcoOtReceiver::encode(u64 otIdx, const void* input, void* dest, u64 destSize)
{
...
void KkrtNcoOtReceiver::configure(bool maliciousSecure, u64 statSecParam, u64 inputBitCount)
{
    if (maliciousSecure)
        throw std::runtime_error(LOCATION);
    if (inputBitCount > 128)
        throw std::runtime_error(
            "currently only support up to 128 bit KKRT inputs. Can be extended on "
            "request" LOCATION);
...
void KkrtNcoOtReceiver::sendCorrection(u64 sendCount, u8* matrix) {
    mHasPendingSendFuture = false;
    // mHasPendingSendFuture = true;
    matrix = (u8*)(mT1.data() + (mCorrectionIdx * mT1.stride()));
...
void KkrtNcoOtReceiver::sendCorrection(Channel& chl, u64 sendCount)
{
    ...
    for (u64 i = mCorrectionIdx; i < mCorrectionIdx + sendCount; ++i)
        if (neq(mT0[i][0], AllOneBlock))
            throw std::runtime_error(
                "This send request contains uninitialized OT. Call encode first...");
#endif
    ...
    mHasPendingSendFuture = true;
    mPendingSendFuture = chl.asyncSendFuture((u8*)(mT1.data() + (mCorrectionIdx * mT1.stride())),
        mT1.stride() * sendCount * sizeof(block));
    // std::cout<<"recvCorrection receiver = "<<((u8*)(mT1.data() + (mCorrectionIdx * mT1.stride())))<<std::endl;
    mCorrectionIdx += sendCount;
}
