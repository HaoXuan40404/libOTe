...
// values have been received by NcoOtExtSender, encode(i\in [0,...,k-1], ...) can be called. This
// will give you the corresponding encoding. Finally, after all correction values have been
...
public:
    ...
    void genBaseOtsWithoutNetwork(PRNG& prng);
    ...
    virtual void setBaseOts(span<block> baseRecvOts, const BitVector& choices, Channel& chl) = 0;
    ...
    virtual void encode(u64 otIdx, const void* choiceWord, void* encoding, u64 encodingSize) = 0;
    ...
    // the sender before they can call encode(otIdx, ...). This method allows this and can be called
    // multiple times so that "streaming" encodes can be performed. The first time it is called, the
    // internal data for otIdx \in {0, 1, ..., recvCount - 1} is received. The next time this
    // function is  called with recvCount' the data for otIdx \in {recvCount, recvCount + 1, ...,
    // recvCount' + recvCount - 1} is sent. The receiver should call sendCorrection(recvCount) with
    // the same recvCount.
    ...
    // An alternative version of the recvCorrection(...) function which dynamically receivers the
    // number of corrections based on how many were sent. The return value is the number received.
...
public:
    ...
    virtual void encode(u64 otIdx, const void* inputWord, void* dest, u64 destSize) = 0;
    ...
    // The way that this class works is that for each encode(otIdx,...), some internal
    // data for each otIdx is generated and stored. This data (corrections) has to be sent to the
    // sender before they can call encode(otIdx, ...). This method allows this and can be called
    // multiple times so that "streaming" encodes can be performed. The first time it is called, the
    // internal data for otIdx \in {0, 1, ..., sendCount - 1} is sent. The next time this function
    // is  called with sendCount' the data for otIdx \in {sendCount, sendCount + 1, ..., sendCount'
    // + sendCount - 1} is sent. The sender should call recvCorrection(sendCount) with the same
    // sendCount.
    // @ chl: the channel that the data will be sent over
