    ...
    for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
    {
        ...
        for (u64 i = 0; i < numCols / 128; ++i)
        {
            for (u64 tIdx = 0, colIdx = i * 128; tIdx < 128; ++tIdx, ++colIdx)
            {
                ...
                mGens[colIdx][0].mAes.ecbEncCounterMode(mGens[colIdx][0].mBlockIdx, superBlkSize,
                    ((block*)t0.data() + superBlkSize * tIdx));
                mGens[colIdx][1].mAes.ecbEncCounterMode(mGens[colIdx][1].mBlockIdx, superBlkSize,
...
void OosNcoOtReceiver::encode(u64 otIdx, const void* input, void* dest, u64 destSize)
{
#ifndef NDEBUG
    if (mInputByteCount == 0)
    ...
    else
    {
        for (u64 i = 0; i < mT0.stride(); ++i)
        {
            ...
            t1Val[i] = codeword[i] ^ t0Val[i] ^ t1Val[i];
        }
...
void OosNcoOtReceiver::configure(bool maliciousSecure, u64 statSecParam, u64 inputBitCount)
{
    if (inputBitCount <= 76)
    {
        ...
        throw std::runtime_error(
            "76 bits is currently the max. larger inputs can be supported on "
            "request.... " LOCATION);
...
void OosNcoOtReceiver::sendFinalization(Channel& chl, block seed)
{
    ...
    for (u64 i = 0; i < mCorrectionIdx; ++i)
    {
        ...
        {
            throw std::runtime_error(
                "All messages must be encoded before check is called. " LOCATION);
        }
