...
void OosNcoOtSender::setBaseOts(span<block> baseRecvOts, const BitVector& choices, Channel& chl)
{
...
void OosNcoOtSender::init(u64 numOTExt, PRNG& prng, Channel& chl)
{
    ...
    for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
    {
        ...
        u64 stopIdx = doneIdx + std::min<u64>(u64(128) * superBlkSize, mT.bounds()[0] - doneIdx);

        for (u64 i = 0; i < numCols / 128; ++i)
        {
            // transpose 128 columns at at time. Each column will be 128 * superBlkSize = 1024 bits
            // long.
            for (u64 tIdx = 0, colIdx = i * 128; tIdx < 128; ++tIdx, ++colIdx)
            {
                // generate the columns using AES-NI in counter mode.
                mGens[colIdx].mAes.ecbEncCounterMode(mGens[colIdx].mBlockIdx, superBlkSize,
...
void OosNcoOtSender::encode(u64 otIdx, const void* plaintext, void* dest, u64 destSize)
{
#ifndef NDEBUG
    if (mInputByteCount == 0)
        throw std::runtime_error("configure must be called first" LOCATION);
    ...
    std::array<block, 10> codeword = {ZeroBlock, ZeroBlock, ZeroBlock, ZeroBlock, ZeroBlock,
        ZeroBlock, ZeroBlock, ZeroBlock, ZeroBlock, ZeroBlock};
...
#else
        // H(x) = AES_f(H'(x)) + H'(x),     where  H'(x) = AES_f(x_0) + x_0 + ... +  AES_f(x_n) +
        // x_n.
...
void OosNcoOtSender::configure(bool maliciousSecure, u64 statSecParam, u64 inputBitCount)
{
    if (inputBitCount <= 76)
    {
...
{
    // receive the next OT correction values. This will be several rows of the form u = T0 + T1 +
    // C(w) there c(w) is a pseudo-random code.
...
void OosNcoOtSender::check(Channel& chl, block seed)
{
    if (mMalicious)
    {
        ...
        recvFinalization(chl);

        // now send them out challenge seed.
        sendChallenge(chl, seed);
        computeProof();
        recvProof(chl);
...
void OosNcoOtSender::recvFinalization(Channel& chl)
{
    // first we need to receive the extra mStatSecParam number of correction
    // values. This will just be for random inputs and are used to mask
    // their true choices that were used in the remaining correction values.
...
void OosNcoOtSender::computeProof()
{
    ...
    for (u64 i = 0; i < mStatSecParam; ++i)
    {
        ...
        {
            qSum[i * mT.stride() + j] =
                (mCorrectionVals[mCorrectionIdx - mStatSecParam + i][j] & mChoiceBlks[j]) ^
    ...
    if (codeSize < zeroAndQ.size())
        throw std::runtime_error("Make this bigger. " LOCATION);
        ...
        if (mT.stride() == 4)
        {
            ...
            for (u64 i = 0; i < stopIdx; ++i, corIter += 4, tIter += 4)
            {
                ...
                for (u64 j = 0; j < 4; ++j)
                {
                    ...
                    // std::cout << "tq " << tq << " cb " << cb << " = c(" << mW_DEBUG[kk][0] << "}
                    // & " << mChoiceBlks[j] << " diff " << (tq ^ cb) << std::endl;
                }
#endif
...
void OosNcoOtSender::recvProof(Channel& chl)
{
    ...
    {
        span<block> word(wSum.data() + l * mCode.plaintextBlkSize(), mCode.plaintextBlkSize());
