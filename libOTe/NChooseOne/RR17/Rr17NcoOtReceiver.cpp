...
void Rr17NcoOtReceiver::init(u64 numOtExt, PRNG& prng, Channel& chl)
{
    ...
    for (u64 step = 0; step < count; ++step)
    {
        ...
        // std::cout << "recv " << *(block*)buff.data() << " c " << count << " s " << curSize <<
        // std::endl; std::cout << "recv " << ((block*)buff.data())[600 * 2] << " c " << count << " s
        // " << curSize << std::endl;

        for (u64 i = 0, j = step * stepSize; i < curSize; ++i, ++j)
        {
            // std::cout << "r kos " << " " << j << " " << mMessages[j] << " " << *choiceIter <<
            // std::endl; std::cout << "r msg " << " " << j << " " << view[i][0] << " " << view[i][1]
            // << std::endl;

            mMessages[j] = mMessages[j] ^ view[i][*choiceIter++];
            // std::cout << "r Msg " << " " << j << " " << mMessages[j] << " " << *choiceIter <<
            // std::endl;
        }
    }
...
void Rr17NcoOtReceiver::encode(u64 otIdx, const void* choiceWord, void* dest, u64 destSize)
{
#ifndef NDEBUG
...
void Rr17NcoOtReceiver::configure(bool maliciousSecure, u64 statSecParam, u64 inputBitCount)
{
