...
void Rr17NcoOtSender::setBaseOts(span<block> baseRecvOts, const BitVector& choices, Channel& chl)
{
...
void Rr17NcoOtSender::init(u64 numOtExt, PRNG& prng, Channel& chl)
{
    ...
    //{
    //    std::cout << "s msg " << i << " " << mMessages[i][0] << " " << mMessages[i][1] <<
    //    std::endl; std::cout << "s kos " << i << " " << view[i][0] << " " << view[i][1] <<
    //    std::endl;
    //}
    ...
    for (u64 step = 0; step < count; ++step)
    {
        ...
        if (step == count - 1)
        {
            ...
            chl.asyncSend(std::move(chunk), [buff]() { delete[] buff; });
        }
        else
        {
...
void Rr17NcoOtSender::encode(u64 otIdx, const void* input, void* dest, u64 destSize)
{
    //#ifndef NDEBUG
...
void Rr17NcoOtSender::configure(bool maliciousSecure, u64 statSecParam, u64 inputBitCount)
{
