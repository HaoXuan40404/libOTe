...
void AknOtReceiver::init(u64 totalOTCount, u64 numberOfOnes, double p, OtExtReceiver& ots,
    span<Channel> chls, PRNG& prng)
{
    auto& chl0 = chls[0];
    ...
    std::shared_future<void> cncGenFuture(cncGenProm.get_future()),
    ...
    auto routine = [&](u64 t, block extSeed, OtExtReceiver& otExt, Channel& chl) {
        ...
        u64 start =
            std::min<u64>(roundUpTo(t * mMessages.size() / chls.size(), 128), mMessages.size());
        u64 end = std::min<u64>(
            roundUpTo((t + 1) * mMessages.size() / chls.size(), 128), mMessages.size());

        span<block> range(mMessages.begin() + start, mMessages.begin() + end);
        ...
        // a buffer of choice bits that were sampled. We aare going to seed these is groups of 4096
        // bits
        ...
        if (t == 0)
        {
            ...
            for (u64 i = 0; i < threadsZeroOnesList.size(); ++i)
            {
                ...
                // memcpy(iter, threadsZeroOnesList[i][1].data(), threadsZeroOnesList[i][1].size() *
                // sizeof(u64));
        ...
        if (t == 1 || chls.size() == 1)
        {
            ...
            for (u64 i = 0; i < threadsZeroOnesList.size(); ++i)
            {
                ...
                // memcpy(iter, threadsZeroOnesList[i][0].data(), threadsZeroOnesList[i][0].size() *
                // sizeof(u64));
