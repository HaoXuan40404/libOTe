...
void AknOtSender::init(u64 totalOTCount, u64 cutAndChooseThreshold, double p, OtExtSender& ots,
    span<Channel> chls, PRNG& prng)
{
    ...
    auto routine = [&](u64 t, block extSeed, OtExtSender& otExt, Channel& chl) {
        ...
        span<std::array<block, 2>> range((mMessages.begin() + start), (mMessages.begin() + end));
    ...
    if (totalOnesCount > cutAndChooseThreshold || neq(proof, totalSum))
    {
        std::cout << "cnc failed. total ones Count = " << totalOnesCount << "  and threshold "
                  << cutAndChooseThreshold << std::endl
