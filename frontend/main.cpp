...
void wedpr_NChooseOne_example_sender(int totalOTs, std::string ip)
{
    std::cout << "Sender" << std::endl;
    auto numOTs = totalOTs;
    auto numChosenMsgs = 1000;
    auto rr = SessionMode::Server;
    IOService ios;
    Session ep0(ios, ip, rr);
    PRNG prng(sysRandomSeed());
    Channel chl = ep0.addChannel();
    NcoOtSender sender;
    ...
    std::cout << "sender genBaseOts" << std::endl;
    ...
    // sender.sendChosen(sendMessages, prng, chl_new);
    // std::cout << "sender sendChosen" << std::endl;
    //   };

    // std::thread thd;
    // std::function<void()> routine = sendRoutine;

    Timer time;
    auto s = time.setTimePoint("start");

    // thd = std::thread(routine);
    // thd.join();
    ...
    Matrix<block> sendMessages(numOTs, numChosenMsgs);
    // prng.get(sendMessages.data(), sendMessages.size());
    prng.get(sendMessagesEach.data(), sendMessagesEach.size());
    for (int i = 0; i < numOTs; i++)
    {
        for (int j = 0; j < numChosenMsgs; j++)
        {
            sendMessages[i][j] = sendMessagesEach[0][j];
            // sendMessages[i][j] = block(char(1), "p", "p", "p", "p","p", "p", "p", "p", "p","p",
            // "p", "p", "p", "p", "p");
        }
        // = sendMessages[0].data();
        // sendMessagesSend[i].set(&sendMessages[0]) ;
    }
    sender.sendChosen(sendMessages, prng, chl);
    for (auto text : sendMessages)
    {
        std::cout << "sender sendMessages iter = " << text << std::endl;
    }
    std::cout << "sender sendChosen" << std::endl;

    auto e = time.setTimePoint("finish");
    auto milli = std::chrono::duration_cast<std::chrono::milliseconds>(e - s).count();
    std::cout << " n=" << totalOTs << " " << milli << " ms" << std::endl;
}

template <typename NcoOtReceiver>
void wedpr_NChooseOne_example_receiver(int totalOTs, std::string ip)
{
    ...
    std::cout << "receiver genBaseOts" << std::endl;
    // aync
    // auto recvRoutine = [&] {
    //   auto &chl_new = chl;
    //   PRNG prng(sysRandomSeed());
    ...
    // recver.receiveChosen(numChosenMsgs, recvMsgs, choices, prng, chl_new);
    //   };
    // std::thread thd;
    // std::function<void()> routine = recvRoutine;
    ...
    for (auto text : recvMsgs)
    {
        std::cout << "========="
                  << "receiver recvMsgs iter = " << text << "=========" << std::endl;
    }

    std::cout << "recver receiveChosen" << std::endl;
}
    ...
    Role role, int totalOTs, int numThreads, std::string ip, std::string tag, CLP&)
{
    ...
    {
        wedpr_NChooseOne_example_receiver<KkrtNcoOtReceiver>(totalOTs, ip);
    }
}

template <typename NcoOtSender, typename NcoOtReceiver>
void wedpr_NChooseOne_example(
    Role role, int totalOTs, int numThreads, std::string ip, std::string tag, CLP&)
{
    const u64 step = 1024;
    ...
    std::cout << "totalOTs=" << totalOTs << std::endl;
    std::cout << "numOTs=" << numOTs << std::endl;
    std::cout << "numThreads=" << numThreads << std::endl;

    // for each thread we need to construct a channel (socket) for it to
    // communicate on.
    ...
    // create a lambda function that performs the computation of a single receiver
    // thread.
    auto recvRoutine = [&](int k) {
        ...
        if (randomOT)
        {
            std::cout << "randomOT recvRoutine" << std::endl;
            ...
            // we must call check. This allows to sender to make sure we did not
            // cheat. For semi-honest protocols, this can and will be skipped.
    ...
    // create a lambda function that performs the computation of a single sender
    // thread.
    auto sendRoutine = [&](int k) {
        ...
        if (randomOT)
        {
            std::cout << "randomOT sendRoutine" << std::endl;
...
void TwoChooseOne_example(
    Role role, int totalOTs, int numThreads, std::string ip, std::string tag, CLP& cmd)
{
    if (totalOTs == 0)
        totalOTs = 1 << 20;
    ...
    // extenders. In real code you would only have a sender or reciever, not both.
    // But we do here just showing the example.
    ...
    auto routine = [&](int i) {
        ...
        if (role == Role::Receiver)
        {
            ...
            {
                // perform  numOTs chosen message OTs, the results will be written to
                // msgs.
    ...
    if (role == Role::Sender)
        lout << tag << " n=" << Color::Green << totalOTs << " " << milli << " ms  " << com
             << " bytes" << std::endl
             << Color::Default;
...
// template<typename OtExtSender, typename OtExtRecver>
void TwoChooseOneG_example(
    Role role, int numOTs, int numThreads, std::string ip, std::string tag, CLP& cmd)
{
    ...
    for (auto s : ss)
        for (auto sec : secs)
            for (auto type : types)
            {
                ...
                if (role == Role::Sender)
                    lout << tag << " n:" << Color::Green << std::setw(6) << std::setfill(' ')
                         << numOTs << Color::Default << " type: " << Color::Green << typeStr
                         << Color::Default << " sec: " << Color::Green << std::setw(3)
                         << std::setfill(' ') << sec << Color::Default << " s: " << Color::Green
                         << s << Color::Default << "   ||   " << Color::Green << std::setw(6)
                         << std::setfill(' ') << milli << " ms   " << std::setw(6)
                         << std::setfill(' ') << com << " bytes" << std::endl
                         << Color::Default;
...
static const std::vector<std::string> unitTestTag{"u", "unitTest"}, kos{"k", "kos"},
    dkos{"d", "dkos"}, kkrt{"kk", "kkrt"}, iknp{"i", "iknp"}, diknp{"diknp"}, oos{"o", "oos"},
    Silent{"s", "Silent"}, akn{"a", "akn"}, np{"np"}, simple{"simplest"}, simpleasm{"simplest-asm"};
...
{
    auto n = cmd.isSet("nn") ? (1 << cmd.get<int>("nn")) : cmd.getOr("n", 0);
    ...
    if (cmd.isSet(tag))
    {
        ...
        else
        {
            auto thrd = std::thread([&] {
                try
                {
                    protocol(Role::Sender, n, t, ip, tag.back(), cmd);
                }
            ...
            try
            {
                protocol(Role::Receiver, n, t, ip, tag.back(), cmd);
            }
            catch (std::exception& e)
            {
                lout << e.what() << std::endl;
...
void minimal()
{
    // Setup networking. See
    // cryptoTools\frontend_cryptoTools\Tutorials\Network.cpp
    IOService ios;
...
const bool wedpr_kkrtEnabled = true;
#else
...
int main(int argc, char** argv)
{
...
#ifdef ENABLE_KKRT
    flagSet |=
        runIf(wedpr_NChooseOne_single_example<KkrtNcoOtSender, KkrtNcoOtReceiver>, cmd, kkrt);
    // flagSet |= runIf(wedpr_NChooseOne_example<KkrtNcoOtSender,
    // KkrtNcoOtReceiver>, cmd, kkrt);
    ...
    if (flagSet == false)
    {
        ...
        std::cout << "Protocols:\n"
                  << Color::Green << "  -simplest-asm" << Color::Default
                  << "  : to run the ASM-SimplestOT active secure  1-out-of-2  base OT   "
                     "   "
                  << Color::Red << (spaEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -simplest    " << Color::Default
                  << "  : to run the SimplestOT     active secure  1-out-of-2  base OT   "
                     "   "
                  << Color::Red << (spEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -np          " << Color::Default
                  << "  : to run the NaorPinkas     active secure  1-out-of-2  base OT   "
                     "   "
                  << Color::Red << (npEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -iknp        " << Color::Default
                  << "  : to run the IKNP           passive secure 1-out-of-2       OT   "
                     "   "
                  << Color::Red << (iknpEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -diknp       " << Color::Default
                  << "  : to run the IKNP           passive secure 1-out-of-2 Delta-OT   "
                     "   "
                  << Color::Red << (diknpEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -Silent      " << Color::Default
                  << "  : to run the Silent         passive secure 1-out-of-2       OT   "
                     "   "
                  << Color::Red << (silentEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -kos         " << Color::Default
                  << "  : to run the KOS            active secure  1-out-of-2       OT   "
                     "   "
                  << Color::Red << (kosEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -dkos        " << Color::Default
                  << "  : to run the KOS            active secure  1-out-of-2 Delta-OT   "
                     "   "
                  << Color::Red << (dkosEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -oos         " << Color::Default
                  << "  : to run the OOS            active secure  1-out-of-N OT for "
                     "N=2^76 "
                  << Color::Red << (oosEnabled ? "" : "(disabled)") << "\n"
                  << Color::Default << Color::Green << "  -kkrt        " << Color::Default
                  << "  : to run the KKRT           passive secure 1-out-of-N OT for "
                     "N=2^128"
                  << Color::Red << (kkrtEnabled ? "" : "(disabled)") << "\n\n"
                  << Color::Default << Color::Green << "  -wedpr        " << Color::Default
                  << "  : to run the wedpr kkrt           passive secure 1-out-of-N OT "
                     "for N=2^128"
                  << Color::Red << (wedpr_kkrtEnabled ? "" : "(disabled)") << "\n\n"
                  << Color::Default

                  << "Other Options:\n"
                  << Color::Green << "  -n           " << Color::Default
                  << ": the number of OTs to perform\n"
                  << Color::Green << "  -r 0/1       " << Color::Default
                  << ": Do not play both OT roles. r 1 -> OT sender and network server. "
                     "r 0 -> OT receiver and network client.\n"
                  << Color::Green << "  -ip          " << Color::Default
                  << ": the IP and port of the netowrk server, default = localhost:1212\n"
                  << Color::Green << "  -t           " << Color::Default
                  << ": the number of threads that should be used\n"
                  << Color::Green << "  -u           " << Color::Default
                  << ": to run the unit tests\n"
                  << Color::Green << "  -u -list     " << Color::Default
                  << ": to list the unit tests\n"
                  << Color::Green << "  -u 1 2 15    " << Color::Default
                  << ": to run the unit tests indexed by {1, 2, 15}.\n"
                  << std::endl;
    }
